...
namespace TrabalhoFinal
{
    public partial class ReturnBorrowing : Form
    {
        ...
        private void btn_return_Click(object sender, EventArgs e)
        {
            ...
            {
                string movie_id = "";
                connectDataBase.SearchBorrowing(txt_seachBorrowing.Text, ref movie_id);
                if (connectDataBase.OpenConnection())
                {
                    tran = connectDataBase.BeginTransaction();
                    connectDataBase.DeleteBorrowingWithoutConnection(txt_seachBorrowing.Text);
                    connectDataBase.updateMovieWithoutConnection(movie_id, "+");                 
                    tran.Commit();
                    MessageBox.Show("Successfully return!!", Util.title, MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            catch(MySqlException ex)
            {
                ...
                {
                    tran.Rollback();
                    MessageBox.Show("Return error!", Util.title, MessageBoxButtons.OK, MessageBoxIcon.Error);
